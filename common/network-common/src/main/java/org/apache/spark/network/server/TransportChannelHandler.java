...
/**
 * The single Transport-level Channel handler which is used for delegating(委派) requests to the
 * {@link TransportRequestHandler} and responses to the {@link TransportResponseHandler}.
 *
 * All channels created in the transport layer are bidirectional(双向). When the Client initiates(发起) a Netty
 ...
 */
/**
 * 一个单向传送级别的channel handler.它被用于委派向TransportRequestHandler请求和响应TransportResponseHandler.
 * 所有的channle被创建在双向的传输层中.当client发起一个RequestMessage(RequestMessage总会被server的RequestHandler获得),
 * Server总会产生一个ResponseMessage(总会被Client的ResponseHandler处理).However, the Server
 * also gets a handle on the same Channel, so it may then begin to send RequestMessages to the
 * Client.
 *
 * 这意味Client需要一个RequestHandler,Server需要ResponseHandler,因为Client响应Server的请求.
 *
 * */
