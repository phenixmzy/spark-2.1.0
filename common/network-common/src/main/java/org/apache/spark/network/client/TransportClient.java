...
/**
 * Client for fetching consecutive(连续到) chunks of a pre-negotiated(预先商定到) stream. This API is intended to(be intended  to 打算/意图为) allow
 ...
 */
/**
 * Client为预定到数据流连续提取数据块.这个API目的是允许大量数据到传输,把数据块大小分成几百KB或MB.
 *
 * 注意,当客户端正在处理提取数据块时,实际启动是在传输层以外完成.函数sendRPC被提供能够控制client和server通讯的执行.
 * TransportClient实体是由TransportClientFactory创建的.一个TransportClient可能被用于多个stream,
 * 但给定的stream都是受限于单一的client,这是为了避免响应失去顺序.
 *
 * 这个类被用于向server发起请求,但它的职责是处理server的响应.同时它也是线程安全的.
 * */
