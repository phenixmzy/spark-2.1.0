 ...
 */
/**
 * 处理来自client的请求和写数据块.每一个handler都隶属于一个Netty Channel,跟踪这个Channel获取的Stream,并在channel结束时清理.
 * streamManager会维护channel与Stream的一一对应.
 * message会有TransportServer通过pipeline的组织进行处理.具体message由handler方法分派处理,不同的msg具体由不同的方法处理.总结大约
 * 分为两类处理：rpcHandler和streamManager.
 * */
