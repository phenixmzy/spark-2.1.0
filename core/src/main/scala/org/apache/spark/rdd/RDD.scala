...
abstract class RDD[T: ClassTag](
   ...
   * be called once, so it is safe to implement a time-consuming(耗时) computation in it.
   */
  ...
  /**
   * Get the array of partitions of this RDD, taking into account（考虑到） whether the
   * RDD is checkpointed or not.
   */
  ...
  /**
    ...
    * 收缩合并分区，减少分区的个数，减小任务调度成本，避免Shuffle导致，
    * 比RDD.repartition效率提高不少。
