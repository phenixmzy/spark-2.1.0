 ...
 */
/**
  * 通过跟踪parent partitions 俘获一个聚合的rdd
  * */
private[spark] case class CoalescedRDDPartition(
   ...
   */
  /**
    * 计算getPreferredLocs中preferredLocation（首选位置） 在 parents' partitions的占比
    * */
 ...
 * so that each new partition has roughly（粗略） the same number of parent partitions and that
 ...
 */
/**
  * 当Spark程序中，存在过多的小任务的时候，可以通过 RDD.coalesce方法，
  * 收缩合并分区，减少分区的个数，减小任务调度成本，避免Shuffle导致，比RDD.repartition效率提高不少。
  * CoalescedRDD一个分区，对应于它parent RDD的那些分区是由PartitionCoalescer数据结构确定的.
  * coalesced RDD的partition比parent RDD要少一点，与parent RDD粗略相同。每个新的partition的preferred location会与
  * parent partitions的preferred locations重叠。
  * */
 ...
 *
 * Furthermore（除此以外）, it is assumed（假定） that the parent RDD may have many partitions, e.g. 100 000.
 ...
 */
/**
  * 合并parent RDD的分区，因此这个rdd的每个分区会计算一个或多个parents。
  * 当parent 超过maxPartitions时，这会明显减少maxPartitions。
  * 当rdd的很多分区并过滤成小分区时，这个转换非常有用。当任务目录下拥有很多文件时，避免大量的小任务。
  * 如果在parent中不是本地信息，合并是非常简单：
  * 如果是是本地信息，则为以下四个目标进行打包：
  * 1）Balance groups，可与parent的分区数粗略相同
  * 2）归档每个本地的分区。比如查找一台大多数parent分区的机器
  * 3）效率。
  * 4）Balance 首选机器。避免打包相同的首选机器。
  * 算法会尝试每台机器分配一个partition。如果期望的partition数大于预选机器数，则需要开始挖掘重复的预选机器。
  * 这个取决于收集评估
  * */
private class DefaultPartitionCoalescer(val balanceSlack: Double = 0.10)
  ...
  /***
    *  one PartitionGroup = one coalesced partition
    *  */
