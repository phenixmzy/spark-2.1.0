...
/**
  * 逻辑rdd与物理存储Blocks的映射关系
  * 物理Blocks均与BlockManager为接口进行存取等操作。
  * BlockManager分为Driver端和Executor端，而BlockManager中的Block有存在于内存，磁盘，甚至需要从远端拷贝。
  * */
