...
class SparkContext(config: SparkConf) extends Logging {
  ...
  try {
    ...
    // retrieve（回复） "HeartbeatReceiver" in the constructor. (SPARK-6640)
   ...
   */
  /**
    * runJob -> dagScheduler.runJob -> dagScheduler.submitJob -> dagScheduler.eventProcessLoop.post(JobSubmitted)
    * */
   ...
   */
  /**
    * 主要清理闭包中不能序列化的变量,防止rdd在网络传输过程中反序列化失败
    * */
