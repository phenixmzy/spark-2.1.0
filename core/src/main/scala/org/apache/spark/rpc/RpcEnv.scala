 ...
 */
/**
  * spark运行时executor可能需要远程下载driver上的jar或文件到本地，对应的内部实现为RpcEnvFileServer，RpcEnvFileServer的子类有NettyStreamManager、HttpBasedFileServer，底层分别由netty、jetty实现，根据参数spark.rpc.useNettyFileServer配置
  *
  * */
private[spark] trait RpcEnvFileServer {
  ...
  /**
   * Adds a local directory to be served(提供) via(通过) this file server.
   *
