...
/**OutboxMessage 依赖TransportClient,通过起接口（本地接口和rpc接口两种）send message，分别由
  * OneWayOutboxMessage
  * RpcOutboxMessage
  * 代理实现
  *
...
private[netty] class Outbox(nettyEnv: NettyRpcEnv, val address: RpcAddress) {
  ...
  /**
    * 发送msg.
    * 如果没有可用的connection,cache msg并运行新的connection.drainOutbox中调用 launchConnectTask 实现;
    * 如果Outbox stop,发送SparkException通知;
    * 正常排放消息;
    * */
  ...
  /**
    * 排放消息队列.
    * 该方法只允许一个thread排放队列中的消息,如果已经有thread正在排放,则后进来的未获得排放权的thread仅仅做return并exit;
    * 获得排放权的thread会一直排放知道队列空元素;
    * */
  ...
  /**
    * 创建客户端连接,并触发排放消息.如果不触发排放消息,当此时没有线程在进行排放,则需要等到下个一个msg到达.
    * */
